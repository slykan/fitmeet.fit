...
function readPoints(xml: string, tagNames: string): { coords: [number, number]; ele: number | null }[] {
  ...
  // self-closing `<trkpt .../>` too and then scans ahead — greedily, via
  // `[\s\S]*?` — for the next real `</trkpt>` anywhere later in the document,
  // silently swallowing every self-closing point in between into unused
  // "content". Files with elevation embedded on only a sample of points
  // (self-closing except every ~Nth) lost ~90% of their points this way.
