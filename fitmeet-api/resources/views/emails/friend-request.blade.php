...
<head>
  ...
  <title>Friend Request — FitMeet</title>
</head>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
          ...
          <tr>
            <td style="background:#16161f;border:1px solid #2a2a3a;border-radius:20px;padding:36px 32px;">
                ...
                <tr>
                  <td align="center">
                    ...
                    @else
                      ...
                      ">{{ strtoupper(substr($sender->name, 0, 1)) }}</div>
                    @endif
                  </td>
                </tr>
              </table>
              ...
              {{-- Heading --}}
              <h1 style="margin:0 0 8px;font-size:20px;font-weight:700;color:#ffffff;text-align:center;line-height:1.3;">
                {{ $sender->name }} wants to connect
              </h1>
              <p style="margin:0 0 28px;font-size:14px;color:#8888aa;text-align:center;line-height:1.5;">
                You received a friend request on FitMeet.
                @if($sender->home_city)
                  <br>{{ $sender->home_city }}{{ $sender->home_country ? ', ' . $sender->home_country : '' }}
                @endif
                @if($sender->skill_level)
                  &nbsp;·&nbsp;{{ ucfirst($sender->skill_level) }}
                @endif
              </p>
          ...
          <tr>
            <td align="center" style="padding-top:28px;">
              <p style="margin:0;font-size:12px;color:#555570;line-height:1.6;">
                You received this because someone on FitMeet sent you a friend request.<br>
